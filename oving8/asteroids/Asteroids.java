...
public class Asteroids extends Pane implements EventHandler<KeyEvent> {
	...
	public void run() {
		Timeline tickTimer = new Timeline(new KeyFrame(Duration.millis(50),
	...
	private void tick(int tickCount) {
		...
		for (int i = 0; i < spaceObjects.size(); i++) {
			for (int j = i + 1; j < spaceObjects.size(); j++) {
				SpaceObject spaceObject1 = spaceObjects.get(i), spaceObject2 = spaceObjects
						.get(j);
		...
		for (int i = 0; i < collisions.size(); i += 2) {
			SpaceObject spaceObject1 = collisions.get(i), spaceObject2 = collisions
					.get(i + 1);
	...
	public void handleGravity(SpaceObject spaceObject1, SpaceObject spaceObject2) {
		if (spaceObject1.getMass() * spaceObject2.getMass() != 0) {
			Point2D d = sub(spaceObject2.getCenter(true),
					spaceObject1.getCenter(true));
			double d2 = d.getX() * d.getX() + d.getY() * d.getY(), dist = Math
					.sqrt(d2);
			double force = spaceObject1.getMass() * spaceObject2.getMass() / d2;
			spaceObject1.applyForce(force * d.getX() / dist, force * d.getY()
					/ dist);
			spaceObject2.applyForce(force * (-d.getX()) / dist,
					force * (-d.getY()) / dist);
		}
	}
	...
	private void handleElasticCollision(SpaceObject so1, SpaceObject so2) {
		...
		double md12 = so1.getMass() - so2.getMass(), ms12 = so1.getMass()
				+ so2.getMass();
	...
	private void handleInelasticCollision(SpaceObject so1, SpaceObject so2) {
		// so1.speedX = (so1.speed * so1.mass + so2.speed * so2.mass) /
		// (so1.mass + s02.mass)
		setSpeed(
				so1,
				div(add(mult(so1.getSpeed(), so1.getMass()),
						mult(so2.getSpeed(), so2.getMass())), so1.getMass()
						+ so2.getMass()));
		remove(so2);
	}
	...
	public static void main(String[] args) {
		AsteroidsProgram.main(new String[] { oving8.asteroids.Asteroids.class
				.getName() });
	}
}