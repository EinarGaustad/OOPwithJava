...
import oving6.cardcontainer.CardHand;
...
public class CardContainerTest extends TestCase {
    ...
    private void testCards(final CardContainer it,
            final Collection<Card> expected) {
    ...
    private void testCards(final Iterator<Card> actual,
            final Collection<Card> expected) {
    ...
    private void _test__handIterator_transitions0_effect_state(
            final CardHand hand) {
        _test__handIterator_transitions0_effect_state_objectTests0_test(hand,
                hand);

    }
    
    private void _test__handIterator_transitions0_effect_state_objectTests0_test(
            final CardHand it, final CardHand hand) {
        try {
            
            this.testCards(it, Collections.<Card> unmodifiableList(Lists
                    .<Card> newArrayList(this.s1, this.c2)));
        } catch (junit.framework.AssertionFailedError error) {
            fail("testCards(#[s1, c2]) failed after hand.addCard(s1) ,hand.addCard(c2): "
                    + error.getMessage());
        }
    ...
    private void _test__deckIterator_transitions0_effect_state_objectTests0_test(
            final CardDeck it, final CardDeck deck) {
        try {
            
            this.testCards(it, Collections.<Card> unmodifiableList(Lists
                    .<Card> newArrayList(this.s1, this.s2, this.h1, this.h2,
                            this.d1, this.d2, this.c1, this.c2)));
        } catch (junit.framework.AssertionFailedError error) {
            fail("testCards(#[s1, s2, h1, h2, d1, d2, c1, c2]) failed: "
                    + error.getMessage());
        }
