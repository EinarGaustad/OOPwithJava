...
public class GridGameGUI extends Application implements GameOutput,
        GridListener {
    ...
    public void start(final Stage stage) throws Exception {
        ...
        GridGame program = (GridGame) Class.forName(parameters.get(0))
                .newInstance();
        init(program, parameters.size() > 1 ? parameters.get(1) : null);
        
        URL url = program.getClass().getResource(
                program.getClass().getSimpleName() + ".fxml");
        if (url == null) {
        ...
        browseButton = lookupNode(root, "#fileButton", Button.class,
                browseActionHandler);
        
        levelTextField = lookupNode(root, "#levelTextField",
                TextInputControl.class, levelTextActionHandler);
        if (levelTextField != null && level != null) {
            levelTextField.setText(level);
        }
        startButton = lookupNode(root, "#startButton", Button.class,
                levelTextActionHandler);
        
        inputTextField = lookupNode(root, "#inputTextField",
                TextInputControl.class, inputTextActionHandler);
        inputButton = lookupNode(root, "#inputButton", Button.class,
                inputTextActionHandler);
        ...
        if (gridPane != null) {
            ...
            for (Node node : root.lookupAll(".keyCombinationId")) {
                keyCombinationNodes.put(
                        KeyCombination.valueOf(node.getId().replace('_', '+')),
                        node);
            }
            for (Node node : root.lookupAll(".inputTextButton")) {
                if (node instanceof Labeled) {
                    char firstChar = ((Labeled) node).getText().charAt(0);
                    KeyCharacterCombination kc = new KeyCharacterCombination(
    ...
    private <T extends Node> T lookupNode(Node root, String selector,
            Class<T> c, EventHandler<ActionEvent> handler) {
    ...
    private <T extends Node> void setActionHandler(T node,
                                                                   ...
                                                                   public void handle(
                                                                       ...
                                                                       getGridGame()
                                                                               .init(levelTextField
                                                                                       .getText());
                                                                       getGridGame()
                                                                               .run(GridGameGUI.this);
                                                                       if (gridPane != null) {
                                                                           gridProvider = getGridGame()
                                                                                   .getGridProvider();
                                                                           gridProvider
                                                                                   .addGridListener(GridGameGUI.this);
                                                                           initGrid();
    ...
    private EventHandler<ActionEvent> inputTextActionHandler   = new EventHandler<ActionEvent>() {
                                                                   ...
                                                                   public void handle(
                                                                           ActionEvent actionEvent) {
                                                                       Object source = actionEvent
                                                                               .getSource();
                                                                       if (source == inputTextField
                                                                               || source == inputButton) {
                                                                           doCommand(inputTextField
                                                                                   .getText());
                                                                       } else if (source instanceof Labeled
                                                                               && ((Node) source)
                                                                                       .getStyleClass()
                                                                                       .contains(
                                                                                               "inputTextButton")) {
                                                                           doCommand(((Labeled) source)
                                                                                   .getText());
                                                                       }
                                                                   }
                                                               };
    ...
    private EventHandler<ActionEvent> browseActionHandler      = new EventHandler<ActionEvent>() {
                                                                   ...
                                                                   public void handle(
                                                                       ...
                                                                       fileChooser
                                                                               .setTitle("Open level file");
                                                                       File file = fileChooser
                                                                               .showOpenDialog(browseButton
                                                                                       .getScene()
                                                                                       .getWindow());
                                                                       if (file != null) {
                                                                           levelTextField
                                                                       ...
                                                                       if (!keyEvent
                                                                           ...
                                                                           if (keyEvent
                                                                               ...
                                                                               directionInput(
                                                                                   ...
                                                                                   .getCode() == KeyCode.DOWN) {
                                                                               directionInput(
                                                                                       0,
                                                                                       1);
                                                                           } else if (keyEvent
                                                                                   .getCode() == KeyCode.LEFT) {
                                                                               directionInput(
                                                                                       -1,
                                                                                       0);
                                                                           } else if (keyEvent
                                                                       ...
                                                                       for (Node child : gridPane
                                                                           ...
                                                                           if (bounds
                                                                                   .contains(
                                                                                           ...
                                                                                           mouseEvent
                                                                                                   .getY())) {
                                                                               Integer column = GridPane
                                                                                       .getColumnIndex(child), row = GridPane
                                                                                       .getRowIndex(child);
                                                                               if (column != null
                                                                                       && column < gridProvider
                                                                                               .getGridWidth()
    ...
    private boolean match(KeyCombination kc, KeyEvent keyEvent) {
        if (keyEvent.getEventType() == KeyEvent.KEY_TYPED) {
            if (kc instanceof KeyCharacterCombination) {
                return ((KeyCharacterCombination) kc).getCharacter().equals(
    ...
    @Override
    public void gridChanged(GridProvider gridProvider, int x, int y, int w,
            int h) {
    ...
    private void initGrid() {
        gridPane.setDimensions(gridProvider.getGridWidth(),
                gridProvider.getGridHeight());
        updateGrid(0, 0, gridProvider.getGridWidth(),
                gridProvider.getGridHeight());
        gridPane.setPrefSize(gridProvider.getGridWidth() * 16,
